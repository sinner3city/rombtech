    ...
    private function _postProcess()
    {
        ...
        if (Tools::getValue('masspricefree_wtd', 1) == 1) {
            if (Tools::getValue('masspricefree_type') == 2 || Tools::getValue('masspricefree_type') == 3) {
                Db::getInstance(_PS_USE_SQL_SLAVE_)->execute('UPDATE `' . _DB_PREFIX_ . 'product_attribute_shop` AS pas ' . $INNER_JOIN . ' SET pas.price=pas.price+pas.price*' . Tools::getValue('masspricefree_value', 0) . '/100 WHERE pas.id_shop="' . Tools::getValue('masspricefree_id_shop') . '"' . $WHERE_PRODUCT_ATTRIBUTE);
            }
            if (Tools::getValue('masspricefree_type') != 3) {
                Db::getInstance(_PS_USE_SQL_SLAVE_)->execute('UPDATE `' . _DB_PREFIX_ . 'product_shop` SET price=price+price*' . Tools::getValue('masspricefree_value', 0) . '/100 WHERE id_shop="' . Tools::getValue('masspricefree_id_shop') . '" ' . $WHERE_PRODUCT);
            }
        } else {
            if (Tools::getValue('masspricefree_type') == 2 || Tools::getValue('masspricefree_type') == 3) {
                Db::getInstance(_PS_USE_SQL_SLAVE_)->execute('UPDATE `' . _DB_PREFIX_ . 'product_attribute_shop` AS pas  ' . $INNER_JOIN . ' SET pas.price=pas.price-pas.price*' . Tools::getValue('masspricefree_value', 0) . '/100 WHERE pas.id_shop="' . Tools::getValue('masspricefree_id_shop') . '"' . $WHERE_PRODUCT_ATTRIBUTE);
