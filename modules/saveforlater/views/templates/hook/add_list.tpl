...
<script type="text/javascript">
    ...
    var ajaxurl = "{$ajaxurl nofilter}"; {*variable contains HTML content, Can not escape this*}
        ...
        {if $kb_sfl_config['saveforlater']['enable'] == 1}
            <span class='bar_item'>
                <span class='velsof_item' id="border_short">
                    ...
                    <span class="bar_text">{*$kb_sfl_config['saveforlater'][$id_lang]|escape:'htmlall':'UTF-8'*}Moje ulubione produkty</span>
                    <span class="circleCount" id="shortlist_count" style="visibility: visible;">{count($shortlisted_products)|intval}</span>
                </span>
                <span class="velsof_popup stored-settings" id="short_popup">
                    <div class="headers">
                        <div class="main_header">
                            <label style='color: {$kb_sfl_config['saveforlater']['color']|escape:'quotes':'UTF-8'}; {if $kb_sfl_config['saveforlater']['italic'] eq 1}font-style: italic;{/if} {if $kb_sfl_config['saveforlater']['bold'] eq 1}font-weight: bold;{/if}'>{*$kb_sfl_config['saveforlater'][{$id_lang|escape:'htmlall':'UTF-8'}]|escape:'quotes':'UTF-8'*}Zapisane produkty</label>
                            <span class="list_count" id="short_count">(<label>{count($shortlisted_products)|intval}</label>)</span>
                            <a title="{l s='Close' mod='saveforlater'}" id="hide_short" class="close_popup">X</a>
                        </div>
                    </div>
                    <div class="velsof_product_list" id="velsof_list">
                        <div class="velsof_container">
                            ...
                            {if count($shortlisted_products) <= 0}
                                <div class="no_data">
                                    <span>Lista ulubionych jest pusta!</span><br>
                                    Dodaj i zapisz swoje ulubione produkty na później!
                                </div>
                            {else}
