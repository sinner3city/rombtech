...
class SaveForLater extends SFLCore
{
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('configuration_form_key') || Tools::getValue('configuration_form_key') == Tools::encrypt(parent::MODULE_KEY)) {
            ...
            $post_data['saveforlater']['color'] = '#000000';
            $post_data['recently_view']['color'] = '';
        ...
        if (count($shortlisted) > 0) {
            foreach ($shortlisted as $id_product) {
                ...
                $shortlisted_products[] = array(
                    ...
                    //line added by rishabh jain
                    'image_url' => Context::getContext()->link->getImageLink($product->link_rewrite, $product->getCoverWs(), ImageType::getFormattedName('small'))
                );
            }
        }
    ...
    public function hookTestowyHook($params)
    {
