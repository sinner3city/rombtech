...
class SFLCore extends Module
{
    ...
    {
        if (!parent::install() || !$this->registerHook('displayTop') || !$this->registerHook('displayHeader') || !$this->registerHook('displayOrderConfirmation') || !$this->registerHook('displayProductPriceBlock') || !$this->registerHook('testowyHook')) {
            return false;
        }
    ...
    {
        if (!parent::uninstall() || !Configuration::deleteByName('KB_SAVE_LATER') || !$this->unregisterHook('displayHeader') || !$this->unregisterHook('displayTop') || !$this->unregisterHook('displayProductPriceBlock') || !$this->unregisterHook('displayOrderConfirmation') || !$this->unregisterHook('testowyHook')) {
            return false;
        }
    ...
    protected function processSettingBeforeSave(&$setting)
    {
        ...
        $setting['recently_view']['bold'] = 0;
        $setting['recently_view']['italic'] = 0;
        $setting['recently_view']['color'] = '#000000';
        $setting['saveforlater']['bold'] = 0;
        $setting['saveforlater']['italic'] = 0;
        $setting['recently_view']['color'] = '#000000';
        ...
        if (!isset($setting['recently_view']['limit'])) {
            $setting['recently_view']['limit'] = 0;
        } else {
            $setting['recently_view']['limit'] = 0;
        }  
