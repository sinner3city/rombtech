...
class MyprestaComment extends ObjectModel
{
    public $id;
    ...
    public static function getByProduct($id_product, $p = 1, $n = null, $id_customer = null)
    {
        ...
        $cache_id = 'MyprestaComment::getByProduct_'.(int)$id_product.'-'.(int)$p.'-'.(int)$n.'-'.(int)$id_customer.'-'.(bool)$validate;
    ...
    {
        $cache_id = 'MyprestaComment::getByCustomer_'.(int)$id_product.'-'.(int)$id_customer.'-'.(bool)$get_last.'-'.(int)$id_guest;
        if (!Cache::isStored($cache_id))
        {
    ...
    {
        /* Get all grades */
        $grades = MyprestaComment::getGradeByProduct((int)$id_product, (int)$id_lang);
        $total = MyprestaComment::getGradedCommentNumber((int)$id_product);
    ...
    public static function getCommentNumber($id_product)
    {
        ...
        $cache_id = 'MyprestaComment::getCommentNumber_'.(int)$id_product.'-'.$validate;
        if (!Cache::isStored($cache_id))
        {
    ...
    {
        $sql  = '
			SELECT pc.`id_product_comment`, pc.`id_product`, IF(c.id_customer, CONCAT(c.`firstname`, \' \',  c.`lastname`), pc.customer_name) customer_name, pc.`title`, pc.`content`, pc.`grade`, pc.`date_add`, pl.`name`, pc.`id_shop`
			FROM `'._DB_PREFIX_.'product_comment` pc
			LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = pc.`id_customer`)
    ...
    public static function getAllActiveCount()
    {
		...
		FROM `'._DB_PREFIX_.'product_comment` pc
		LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = pc.`id_customer`)
		INNER JOIN `'._DB_PREFIX_.'product_lang` pl ON (pl.`id_product` = pc.`id_product` AND pl.`id_lang` = '.(int)Context::getContext()->language->id.Shop::addSqlRestrictionOnLang('pl').')
		WHERE 1=1 '.((int)Configuration::get('PRODUCT_COMMENTS_MODERATE') == 1 ? ' && pc.`validate` = 1':'')));
    }
    ...
    {
        return (Db::getInstance()->executeS('
		SELECT pc.`title`, pc.`id_product_comment`, pc.`id_product`, IF(c.id_customer, CONCAT(c.`firstname`, \' \',  c.`lastname`), pc.customer_name) customer_name, pc.`content`, pc.`grade`, pc.`date_add`, pl.`link_rewrite`, pl.`name`
		FROM `'._DB_PREFIX_.'product_comment` pc
		LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = pc.`id_customer`)
		INNER JOIN `'._DB_PREFIX_.'product_lang` pl ON (pl.`id_product` = pc.`id_product` AND pl.`id_lang` = '.(int)Context::getContext()->language->id.Shop::addSqlRestrictionOnLang('pl').')
		WHERE 1=1 '.((int)Configuration::get('PRODUCT_COMMENTS_MODERATE') == 1 ? ' && pc.`validate` = 1':'').'
		ORDER BY pc.`date_add` DESC LIMIT '.(($page-1)*$nb).','.$nb));
    }

    public static function getAllActivePro($id_product, $p = 1, $n = null)
    {
        $id_customer = 0;
        $validate = Configuration::get('PRODUCT_COMMENTS_MODERATE');
        $p = (int)$p;
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            ((int)$id_customer ? '(SELECT count(*) FROM `'._DB_PREFIX_.'product_comment_report` pcrc WHERE pcrc.`id_product_comment` = pc.`id_product_comment` AND pcrc.id_customer = '.(int)$id_customer.') as customer_report, ' : '').'
			IF(c.id_customer, CONCAT(c.`firstname`, \' \',  LEFT(c.`lastname`, 1)), pc.customer_name) customer_name, pc.`content`, pc.`grade`, pc.`date_add`, pc.title, 
			pc.`id_product_comment`, pc.`id_product`, IF(c.id_customer, CONCAT(c.`firstname`, \' \',  c.`lastname`), pc.customer_name) customer_name, pc.`content`, pc.`grade`, pc.`date_add`, pl.`link_rewrite`, pl.`name`
			  FROM `'._DB_PREFIX_.'product_comment` pc
			LEFT JOIN `'._DB_PREFIX_.'customer` c ON c.`id_customer` = pc.`id_customer`
			INNER JOIN `'._DB_PREFIX_.'product_lang` pl ON (pl.`id_product` = pc.`id_product` AND pl.`id_lang` = '.(int)Context::getContext()->language->id.Shop::addSqlRestrictionOnLang('pl').')
			'.((int)Configuration::get('PRODUCT_COMMENTS_MODERATE') == 1 ? ' WHERE  pc.`validate` = 1':'').'
			AND pc.`id_shop` = ' . (int)Context::getContext()->shop->id . '
			ORDER BY pc.`date_add` DESC
    ...
    public function validate($validate = '1')
    {
        ...
        Hook::exec('actionObjectMyprestaCommentValidateAfter', array('object' => $this));
        return $success;
    }
    ...
    {
        parent::delete();
        MyprestaComment::deleteGrades($this->id);
        MyprestaComment::deleteReports($this->id);
        MyprestaComment::deleteUsefulness($this->id);
    }
