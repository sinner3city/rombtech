...
/**
 * CUSTOM_CSSJSUpdater updater class for the module.
 */
class CustomCustomcssupdater {
	...
	public function __construct() {
        ...
        $todate        = Configuration::get( 'CUSTOM_CSSJS_DATE' );

        $stable = Configuration::get( 'CUSTOM_CSSJS_STABLE' );
        $d_link = Configuration::get( 'CUSTOM_CSSJS_DLINK' );

        if ( isset( $stable ) && isset( $d_link ) ) {
            if ( $stable == '' && $d_link == '' ) {
                $today = date( 'Y-m-d' );
                if ( $today > $todate ) {
                    Configuration::updateValue( 'CUSTOM_CSSJS_DATE', $today );
                    $this->customcssjs_get_update();
                    Configuration::updateValue( 'CUSTOM_CSSJS_STABLE', '' );
			        Configuration::updateValue( 'CUSTOM_CSSJS_DLINK', '' );
                }
            } else {
	...
	/**
	 * CUSTOM_CSSJS_get_update checks the api if the module has update available.
	 */
	public function customcssjs_get_update() {
		$api_params = array(
			'edd_action' => 'get_version',
			'item_id'    => $this->product_id,
			'version'    => CUSSSTOM_CSSJS_VERSION,
	...
	private function show_notification( $v, $d ) {
		$msg = 'There is a new version of Custom Product Extra Tab is available.';
        return;
        ?>
<div class="row">
    <div class="col-lg-12">
