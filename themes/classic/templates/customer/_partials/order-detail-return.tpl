...
{block name='order_products_table'}
  ...
  <form id="order-return-form" action="{$urls.pages.order_follow}" method="post">
    ...
    <div class="order-totals hidden-md-up box">
      ...
      {/foreach}
      <div class="order-total row">
        <div class="col-xs-8"><strong>{$order.totals.total_including_tax.label}</strong></div>
        <div class="col-xs-4 text-xs-right">{$order.totals.total_including_tax.value}</div>
      </div>
    </div>
