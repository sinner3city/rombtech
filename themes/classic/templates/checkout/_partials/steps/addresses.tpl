...
{block name='step_content'}
  <div class="js-address-form">
    ...
    >
        ...
        <h3 class="address__title">
          <span class="relative">
            {l s='Shipping Address' d='Shop.Theme.Checkout'}
              ...
              <a href="{$new_address_delivery_url}" class="add-address__add-link"><i class="material-icons">&#xE145;</i>{l s='add new address' d='Shop.Theme.Actions'}</a>
      ...
      {*if $use_same_address && !$cart.is_virtual}
      <strong class="t-center display--block pd-b">
        <a data-link-action="different-invoice-address" href="{$use_different_address_url}">
          {l s='Billing address differs from shipping address' d='Shop.Theme.Checkout'}? Kliknij i dodaj
        </a>
      </strong>
    {/if*}
      {if $show_delivery_address_form}
        <div id="delivery-address" class=" flex-row-center">
      ...
      {elseif $customer.addresses|count > 0}
        <div id="delivery-addresses" class="address-selector js-address-selector flex-row-center">
      ...
      {if !$use_same_address}
        ...
        <h3 class="address__title">
          <span class="relative"> 
            ...
            <a href="{$new_address_invoice_url}" class="add-address__add-link"><i class="material-icons">&#xE145;</i>{l s='add new address' d='Shop.Theme.Actions'}</a>
        ...
        {if $show_invoice_address_form}
          <div id="invoice-address" class=" flex-row-center">
            {render file                      = 'checkout/_partials/address-form.tpl'
              ui                        = $address_form
              use_same_address          = $use_same_address
              type                      = "invoice"
        ...
        {else}
          <div id="invoice-addresses" class="address-selector js-address-selector flex-row-center">
            {include  file        = 'checkout/_partials/address-selector-block.tpl'
              addresses   = $customer.addresses
      ...
      {if !$form_has_continue_button}
        <div class="t-center">
          <button type="submit" class="btn btn--primary btn--large continue float-xs-right" name="confirm-addresses" value="1">
