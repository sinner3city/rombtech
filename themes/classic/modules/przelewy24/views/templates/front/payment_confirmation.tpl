...
{block name='page_content'}
    <div class="p24-payment-return-page przelewy-24 t-center">
        ...
        <a href="http://przelewy24.pl" target="_blank">
            <img src="{$logo_url}" alt="{l s='Pay with Przelewy24' mod='przelewy24'}">
        </a>
        {if 'ok' === $status}
                ...
                {if $customer_cards|sizeof > 0}
                    <div class="p24-recurring">
                        ...
                        {foreach from=$customer_cards key=id item=card}
                            <a class="bank-box recurring" data-id="{$card.card_type_md5}" data-card-id="{$card.id}"
                    ...
                    <div class="">
                        <label>
                            <input data-action="{$p24_ajax_notices_url}" {if $remember_customer_cards}checked="checked" {/if}
                                type="checkbox" id="p24-remember-my-card" class="p24-remember-my-card" name="p24-remember-my-card"
                                value="1">
            ...
            {if $P24_PAYMENT_METHOD_LIST}
                {if $p24_paymethod_graphics}
                    <div class="pay-method-list pay-method-graphics pay-method-list-first">
                        {if $p24_paymethod_list_first|sizeof > 0}
                            {foreach $p24_paymethod_list_first as $bank_id => $bank_name}
                                {if !empty($bank_name)}
                                    <a class="bank-box"
                                        {if (145 === ($bank_id|intval) || 218 === ($bank_id|intval)) && $pay_card_inside_shop}onclick="showPayJsPopup()"
                                        {/if} data-id="{$bank_id}">
                    ...
                    <div class="pay-method-list pay-method-graphics pay-method-list-second">
                ...
                {else}
                                ...
                                {if !empty($bank_name)}
                                    <li>
                                        <input type="radio" class="bank-box"
                                            {if (145 === ($bank_id|intval) || 218 === ($bank_id|intval)) && $pay_card_inside_shop}onclick="showPayJsPopup()"
                                            {/if} data-id="{$bank_id}" id="paymethod-bank-id-{$bank_id}" name="paymethod-bank">
                    ...
                    <ul class="pay-method-list pay-method-text-list pay-method-list-second">
                        {if $p24_paymethod_list_second|sizeof > 0}
                            {foreach $p24_paymethod_list_second as $bank_id => $bank_name}
                                {if !empty($bank_name)}
                                    <li>
            ...
            <form action="{$p24_url}" method="post" id="przelewy24Form" name="przelewy24Form" accept-charset="utf-8">
                ...
                {if 'blik_uid' === $p24_method}
                    ...
                    <div id="blikCodeContainer" class="form-group" {if $p24_blik_alias}style="display: none;" {/if}
                        data-websocket-url="{$p24_blik_websocket}" data-shop-order-id="{$p24_shop_order_id}"
                        data-payment-failed-url="{$p24_payment_failed_url}" data-ajax-verify-blik-url="{$p24_blik_ajax_verify_url}"
                ...
                {if $payment_method_selected_id > 0}
                    <button data-validation-required="{$validationRequired}"
                        data-text-oneclick="{l s='Pay by OneClick' mod='przelewy24'}" {$submitButtonDisabled} id="submitButton"
                        class="btn btn-primary" {if 2 === ($validationRequired|intval)}data-validation-link="{$validationLink}"
                        type="button" onclick="proceedPayment({$cartId})" {else} onclick="formSend();" type="submit"
                        {/if}>{l s='Pay' mod='przelewy24'}</button>
                ...
                {else}
                    <button {$submitButtonDisabled} id="submitButton" data-validation-required="{$validationRequired}"
                        data-text-oneclick="{l s='Pay by OneClick' mod='przelewy24'}" class="btn btn-primary"
                        {if 2 === ($validationRequired|intval)}data-validation-link="{$validationLink}" type="button"
                        onclick="proceedPayment({$cartId})" {else} onclick="formSend();" type="submit"
                        {/if}>{l s='Pay by Przelewy24' mod='przelewy24'}</button>
                {/if}
            </form>
        ...
        {else}
            <p class="warning">
                ...
                <a href="https://fshop.pl/demo/info/13-kontakt-z-nami"
                    target="_blank">{l s='customer service' mod='przelewy24'}</a>.
            </p>
        {/if}
    </div>
    ...
    <div class="row p24-register-card-wrapper" data-dictionary='{
                    ...
                    }' data-card-action="cardCharge" data-card-cart-id="{$cartId}"
        data-successCallback="payInShopSuccess" data-failureCallback="payInShopFailure"
