        ...
        <div class="row">
          <div class="col-text text-left">
            <div class="flex-row-center flex-wrap">
              <div class="col-md-6 row-info">
                <h6 class="h6 product-name">{$product.name}</h6>
                {hook h='displayProductPriceBlock' product=$product type="unit_price"}
                {foreach from=$product.attributes item="property_value" key="property"}
                  <span
                    class="prod-atr"><strong>{if $property_value !== "12" && $property_value !== "6" && $property_value !== "3"}{$property_value}{/if}{if $property_value == "12"}Abonament:
                      <span class="mc-label">Roczny</span>{/if}{if $property_value == "6"}Abonament: <span
                        class="mc-label">Półroczny</span>{/if}{if $property_value == "3"}Abonament: <span
                      class="mc-label">Kwartalny</span>{/if}</strong></span><br>
                {/foreach}
          ...
          <div class="col-md-7">
            <div class="cart-content">
              ...
              <div class="cart-content-btn flex-wrap">
                <button type="button" class="btn btn-secondary"
                  data-dismiss="modal">{l s='Powrót' d='Shop.Theme.Actions'}</button>
                <a href="{$cart_url}" class="btn btn-primary  btn--icon">

                  <i
                    class="material-icons rtl-no-flip">credit_score</i>{l s='Proceed to checkout' d='Shop.Theme.Actions'}</a>
              </div>
            </div>
          </div>
        </div>
